...
function validateHeight(){
    ...
    if(str==''){    
       ...
       flag=false;
            ...
            $("#height").attr("data-toggle", "tooltip"); 
            $("#height").attr("title", "Height can contain only digits");
            flag=false;
        }
    }
}
...
function validateWeight(){
    ...
    if(str==''){    
       ...
       flag=false;
            ...
            if(!(match != null && str == match[0])){
            $("#weight").attr("data-toggle", "tooltip"); 
            $("#weight").attr("title", "Weight can contain only digits");
            flag=false;
        }
    }
}
...
function getJson(){
	var $items = $('#cid,#name, #address,#city,#state,#zipcode,#contactno,#email,#dob,#mark,#height,#weight,#premiums,#income,#password');
	var obj = {};
	$items.each(function() {
...
function redir(){
	var json=getJson();
	$.post("http://localhost:6843/customer/register",json,
